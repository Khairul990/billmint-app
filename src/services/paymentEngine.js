...
import { getInvoicePaymentStatus, calculateCanonicalInvoiceFinancials } from '../utils/invoiceMath.js';
...
class PaymentEngine {
  async addPayment(invoiceId, paymentData = {}) { return await invoiceEngine.markAsPaid(invoiceId, paymentData); }

  async removePayment(invoiceId, paymentId) {
    const invoices = await invoiceEngine.getInvoices(true);
    const invoice = invoices.find(inv => inv.id === invoiceId);
    if (!invoice) return null;
    invoice.paymentHistory = Array.isArray(invoice.paymentHistory) ? invoice.paymentHistory.filter(p => p.id !== paymentId && p.proofId !== paymentId) : [];
  ...
  calculateTotalPaid(payments = []) { return payments.reduce((sum, p) => sum + (Number(p.amount) || 0), 0); }

  async recordPaymentProof(invoiceId, proofData) {
    return await this.addPayment(invoiceId, { amount: proofData.amount, method: proofData.method || 'Transfer', date: new Date().toISOString(), proofUrl: proofData.url, transactionId: proofData.transactionId || '' });
  }

  getAllTransactions(invoices = []) {
    const transactions = [];
    invoices.forEach(inv => {
      (inv.payments || []).forEach(p => transactions.push({ ...p, invoiceId: inv.id, invoiceNumber: inv.invoiceNumber, customerName: inv.customer?.name || inv.customerName || 'Unknown' }));
      (inv.paymentHistory || []).forEach(p => transactions.push({ ...p, invoiceId: inv.id, invoiceNumber: inv.invoiceNumber, customerName: inv.customer?.name || inv.customerName || 'Unknown' }));
    });
    return transactions.sort((a, b) => new Date(b.date) - new Date(a.date));
  }

  async submitPlatformPaymentProof(proofData) { return await dbSubmitPlatformPaymentProof(proofData); }
  ...
  async approvePaymentProof(payment) {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      transaction.update(proofRef, { status: 'approved', updatedAt: new Date().toISOString() });
      ...
      if (!alreadyRecorded) history.push({ id: `pmt_${payment.id}`, proofId: payment.id, amount: paymentAmount, method: payment.paymentMethod || 'Proof Approval', transactionId: payment.transactionId || '', date: new Date().toISOString(), note: payment.notes || payment.note || 'Payment proof approved', source: 'payment_proof' });

      const canonical = calculateCanonicalInvoiceFinancials({ ...publicData, paymentHistory: history });
      transaction.update(publicInvRef, { paymentHistory: history, amountPaid: canonical.amountPaid, paidAmount: canonical.amountPaid, balanceDue: canonical.balanceDue, paymentStatus: canonical.paymentStatus, status: canonical.paymentStatus, updatedAt: new Date().toISOString() });
    });
    ...
    if (payment.invoiceId) {
      ...
      if (existingInvoice) {
        ...
        const alreadyRecorded = history.some(p => p.proofId === payment.id || p.id === payment.id || p.id === `pmt_${payment.id}`);
        if (!alreadyRecorded) history.push({ id: `pmt_${payment.id}`, proofId: payment.id, amount: paymentAmount, method: payment.paymentMethod || existingInvoice.paymentMethod || 'Proof Approval', transactionId: payment.transactionId || '', date: new Date().toISOString(), note: payment.notes || payment.note || 'Payment proof approved', source: 'payment_proof' });

        const normalized = await invoiceEngine.saveInvoice({ ...existingInvoice, paymentHistory: history, paymentMethod: payment.paymentMethod || existingInvoice.paymentMethod || 'UPI' });
        try {
          const { bankEngine } = await import('./bankEngine.js');
          const entry = history.find(p => p.proofId === payment.id || p.id === `pmt_${payment.id}`);
          if (entry) await bankEngine.autoPostPayment({ id: entry.id, amount: entry.amount, method: entry.method, date: entry.date, invoiceId: normalized.id, invoiceNumber: normalized.invoiceNumber, customerId: normalized.customer?.id || normalized.customerId || null, customerName: normalized.customer?.name || normalized.customerName || '', note: entry.note });
        } catch (e) { console.warn('[BANK] auto-post proof payment skipped:', e); }
        if (typeof window !== 'undefined') {

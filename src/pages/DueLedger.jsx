              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-2 shrink-0 border-t lg:border-t-0 pt-3 lg:pt-0 border-theme-border-soft/50">
                  <button
                    ...
                    title={t('dl.collect_title', 'Collect Payment in Collection Center')}
                  >
                    <Banknote className="w-3.5 h-3.5" />
                    <span>{t('dl.collect_money', 'Collect Money')}</span>
                  </button>
                  ...
                  <button
                    ...
                    title={t('dl.remind_title', 'Send WhatsApp Reminder')}
                  >
                    <Send className="w-3.5 h-3.5" />
                    <span className="hidden xl:inline text-xs">{t('dl.remind', 'Remind')}</span>
                  </button>
                  ...
                  <button
                    ...
                    title={t('dl.ledger_title', 'View Customer 360° Ledger')}
                  >
                    <Eye className="w-3.5 h-3.5" />
                    <span className="hidden xl:inline text-xs">{t('dl.ledger', 'Ledger')}</span>
                  </button>
                </div>
              </div>
            </SignatureSurface>
    ...
    <div className="page-premium w-full max-w-full pb-28 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-theme-primary tracking-tight">
            {t('dl.title', 'Due Ledger')}
          </h1>
          <p className="text-xs font-semibold text-theme-muted mt-0.5">
            {t('dl.subtitle', 'Money Still to Collect · Real-time debtor intelligence & collection pipeline')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <Download className="w-4 h-4" /> {t('dl.export_due', 'Export Due List')}
          </button>
        </div>
      </div>
        ...
        <SignatureSurface variant="neutral" className="p-4 sm:p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-black uppercase tracking-wider text-theme-muted">
              {t('dl.total_outstanding', 'Total Outstanding')}
            </span>
            <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 font-numbers">
              {dueBills.length} {t('dl.bills', 'bills')}
            </span>
          </div>
          <FinancialValue 
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('dl.search_ph', 'Search by customer name, phone, or invoice number...')}
      ...
      {!searchQuery && dueBills.length === 0 && (
        <SignatureSurface variant="neutral" className="p-8 text-center space-y-3">
          ...
          <h3 className="text-base font-black text-theme-primary">{t('dl.all_settled', 'All Accounts Settled!')}</h3>
          <p className="text-xs text-theme-muted max-w-sm mx-auto leading-relaxed">
            {t('dl.all_settled_sub', 'There are no pending balances or overdue bills across any customer account.')}
          </p>
        </SignatureSurface>
      )}
      ...
      {searchQuery && allFilteredEmpty && (
        <SignatureSurface variant="neutral" className="p-8 text-center space-y-3">
          ...
          <h3 className="text-base font-black text-theme-primary">{t('dl.no_match', 'No Matching Due Bills')}</h3>
          <p className="text-xs text-theme-muted max-w-sm mx-auto leading-relaxed">
            {t('dl.no_match_sub', 'No bills match "{q}". Try searching by customer name or bill number.').replace('{q}', searchQuery)}
          </p>
        </SignatureSurface>
      )}

...
const SettingsStudioV2 = ({
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" className="settings-studio-premium min-h-screen pb-32 font-sans w-full bg-theme-app text-theme-primary">
      ...
      {/* Mobile Section Chip Bar — every settings section reachable on phones */}
      <div className="md:hidden sticky top-[53px] z-20 bg-theme-app/95 backdrop-blur-xl border-b border-theme-border-soft">
        <div className="flex items-center gap-2 px-3 pt-2.5">
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => handleSearchChange(e.target.value)}
              placeholder="Search settings..."
              className="w-full pl-9 pr-8 py-2 bg-theme-surface/70 border border-theme-border-soft rounded-xl text-xs font-semibold text-theme-primary placeholder:text-theme-muted focus:outline-none focus:ring-2 focus:ring-theme-accent/20 focus:border-theme-accent transition-all"
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-theme-muted hover:text-theme-primary cursor-pointer">
                <X className="w-3 h-3" />
          ...
          <div className="flex bg-theme-surface p-0.5 rounded-xl border border-theme-border-soft text-[10px] font-bold shrink-0">
            {['All', 'Simple', 'Advanced'].map((mode) => (
              <button
                key={mode}
                onClick={() => setSettingsMode(mode)}
                className={`px-2 py-1.5 rounded-lg transition-all cursor-pointer ${
                  settingsMode === mode ? 'bg-theme-card text-theme-primary shadow-xs' : 'text-theme-muted'
                }`}
              >
                {mode}
              </button>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 overflow-x-auto no-scrollbar">
          {NAV_GROUPS.flatMap((group) =>
            group.items
              .filter((item) => settingsMode === 'All' || item.mode === settingsMode.toLowerCase())
              .map((item) => {
                const Icon = item.icon;
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => setActiveSection(item.id)}
                    className={`flex items-center gap-1.5 whitespace-nowrap rounded-full px-3.5 py-2 text-[11px] font-bold transition-all cursor-pointer border ${
                      isActive
                        ? 'bg-[image:var(--accent-gradient)] text-white border-transparent shadow-md'
                        : 'bg-theme-surface text-theme-muted border-theme-border-soft hover:text-theme-primary'
                    }`}
                  >
                    <Icon className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-white' : 'text-theme-muted'}`} />
                    {item.label}
                  </button>
                );
              })
      ...
      {/* Mobile: current section context header */}
      <div className="md:hidden px-4 pt-3">
        <p className="text-[11px] font-semibold text-theme-muted leading-snug">
          {currentSectionItem?.description || 'Adjust your workspace preferences.'}
        </p>
      </div>
    </motion.div>
  );

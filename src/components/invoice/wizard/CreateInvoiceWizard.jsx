...
import InvoiceWorkAllocationStep from './steps/InvoiceWorkAllocationStep';
...
const STEPS = [
  ...
  { id: 3, title: '৩. কাজের হিসাব' },
  { id: 4, title: '৪. পেমেন্ট ও ডিসকাউন্ট' },
  { id: 5, title: '৫. প্রিভিউ ও সেভ' }
];
...
const CreateInvoiceWizard = ({ onSaveInvoice, onDownloadPDF, setCurrentTab, customers, products }) => {
  ...
  const handleSave = (statusOverride, isSilent = false) => {
    ...
    // Keep work attribution attached to the invoice itself. This makes the
    // revenue/cost breakdown durable even when modules are later disabled.
    const validItemIds = new Set(cleanedItems.map(item => item.id));
    const workAllocations = (Array.isArray(state.workAllocations) ? state.workAllocations : [])
      .filter(a => validItemIds.has(a.itemId))
      .map(a => ({
        ...a,
        costAmount: Math.max(0, Number(a.costAmount) || 0)
      }));

    const staffWorkTotal = workAllocations
      .filter(a => a.type === 'staff')
      .reduce((sum, a) => sum + (Number(a.costAmount) || 0), 0);
    const outsourceWorkTotal = workAllocations
      .filter(a => a.type === 'outsource')
      .reduce((sum, a) => sum + (Number(a.costAmount) || 0), 0);
    const allocatedWorkCost = Math.round((staffWorkTotal + outsourceWorkTotal) * 100) / 100;
    ...
    const payload = {
      ...
      outsourceWorkTotal,
      allocatedWorkCost,
      ownWorkTotal: Math.max(0, (Number(state.totals.grandTotal) || 0) - allocatedWorkCost),
    ...
    if (!isSilent && statusOverride !== 'Draft') toast.success('Invoice saved successfully!');
  };

  const handleNext = () => {
    if (currentStep === 1 && !state.customer.name) {
    ...
    <div className="flex flex-col min-h-screen pb-24 md:pb-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm font-bold text-theme-muted mt-1">Step {currentStep} of {STEPS.length}: {STEPS[currentStep-1].title}</p>
        </div>
          ...
          return (
            <div key={step.id} className="relative z-10 flex flex-col items-center">
              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                isCompleted ? 'bg-theme-accent text-white scale-100 shadow-lg shadow-theme-glow' :
                isCurrent ? 'bg-theme-card border-2 border-theme-accent text-theme-accent scale-110 shadow-xl' :
                'bg-theme-surface border-2 border-theme-border-soft text-theme-muted scale-95'
              }`}>
                {isCompleted ? <Check className="w-5 h-5" /> : step.id}
              </div>
              <span className={`absolute top-12 text-center w-20 md:w-32 text-[10px] md:text-xs font-bold transition-colors duration-300 ${
      ...
      <div className="flex-1">
        <AnimatePresence mode="wait">
          <motion.div key={currentStep} variants={containerVariants} initial="hidden" animate="show" exit="exit" className="h-full">
            {currentStep === 1 && <CustomerSelectionStep customers={customers} />}
            {currentStep === 2 && <ItemsSelectionStep products={products} />}
            {currentStep === 3 && <InvoiceWorkAllocationStep />}
            {currentStep === 4 && <PaymentDiscountStep />}
            {currentStep === 5 && <PreviewDownloadStep onDownloadPDF={onDownloadPDF} handleSave={handleSave} />}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:relative md:mt-8 bg-theme-card border-t border-theme-border-soft md:border-none p-4 md:p-0 z-40 md:bg-transparent shadow-[0_-10px_30px_rgba(0,0,0,0.05)] md:shadow-none">
        <div className="max-w-7xl mx-auto flex flex-col-reverse md:flex-row justify-between gap-3 md:gap-4">
          <button
            ...
            className={`w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-bold transition-all min-h-[48px] ${currentStep === 1 ? 'opacity-50 cursor-not-allowed bg-theme-surface text-theme-muted' : 'bg-theme-surface hover:bg-theme-border-soft text-theme-primary'}`}
          >
            <ArrowLeft className="w-5 h-5" /> Previous
          </button>

          {currentStep < STEPS.length ? (
            <ShimmerButton onClick={handleNext} className="w-full md:w-auto min-h-[48px] shadow-lg shadow-theme-glow" shimmerColor="#ffffff" background="var(--accent-gradient)">
              <div className="flex items-center gap-2 text-theme-button-text font-black">Next <ArrowRight className="w-5 h-5" /></div>
            </ShimmerButton>
          ) : (
            <ShimmerButton onClick={() => handleSave('Pending')} className="w-full md:w-auto min-h-[48px] shadow-lg" shimmerColor="#ffffff" background="var(--status-success)">
              <div className="flex items-center gap-2 text-white font-black"><Save className="w-5 h-5" /> Finalize Invoice</div>
            </ShimmerButton>
          )}
        </div>
      </div>

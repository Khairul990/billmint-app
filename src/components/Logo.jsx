...
const Logo = ({ type = 'horizontal', className = '', forceWhiteText = false }) => {
  ...
  const defs = (
    <defs>
      {/* Removed failing brandGradient, using currentColor and text-theme-accent instead */}
  ...
  if (type === 'app-icon') {
    return (
      <div className={`aspect-square rounded-[2rem] bg-theme-card dark:bg-theme-card dark:bg-theme-app flex flex-col items-center justify-center p-6 border border-theme-border-soft dark:border-theme-border-soft dark:border-theme-border-soft shadow-premium transition-all duration-300 hover:shadow-premium-hover ${className}`}>
        <IconSVG sizeClass="w-3/5 h-3/5" />
        <div className="mt-4 flex flex-col items-center select-none text-center">
          <span className="text-xl font-extrabold text-theme-primary dark:text-theme-primary dark:text-theme-primary tracking-tight transition-colors duration-300">
            Bill<span className="text-theme-accent dark:text-theme-accent font-extrabold">Qyro</span>
          </span>
          <span className="text-[6.5px] font-black uppercase text-theme-muted dark:text-theme-muted tracking-widest mt-1.5 flex items-center gap-1 leading-none">
    ...
    >
      ...
      <div className="flex flex-col select-none">
        <div className="flex items-baseline leading-none">
          {/* Bill text */}
          <motion.span 
            className={`text-xl font-black ${forceWhiteText ? 'text-white' : 'text-theme-primary dark:text-theme-primary'} tracking-tight`}
            variants={{
          ...
          <motion.span 
            className="text-xl font-black text-theme-accent dark:text-theme-accent tracking-tight relative pr-3"
            variants={{
              hidden: { opacity: 0, x: -5 },
        ...
        <motion.span 
          className={`text-[6.5px] font-black uppercase ${forceWhiteText ? 'text-theme-muted' : 'text-theme-muted dark:text-theme-muted'} tracking-widest mt-1.5 flex items-center gap-1.5 leading-none`}
          variants={{

...
export const InvoiceProvider = ({ children, editingInvoice, invoices, businessSettings }) => {
  ...
  useEffect(() => {
    ...
    if (editingInvoice) {
      ...
      dispatch({ type: 'INIT_INVOICE', payload: { id: 'inv-' + Date.now(), publicToken: invoiceEngine.generateSecureToken(), invoiceNumber: generateNextInvoiceNumber(invoices), date: today, dueDate: due, billType: businessSettings?.defaultBillingTemplate || 'custom', selectedTemplate: 'retail', templateFields: invoiceTemplates[0].fields, pdfVisibleFields: businessSettings?.pdfVisibleFields?.[businessSettings?.defaultBillingTemplate || 'custom'] || [], items: [{ id: `item-${Date.now()}-0`, sn: 1, designNo: '', workType: 'Embroidery', description: '', size: '', qty: 1, rate: 0, amount: 0, smartRate: { repair: 0, punching: 0, embroidery: 0, other: 0 } }], totals: { subtotal: 0, taxPercentage: businessSettings?.defaultTax ?? 0, taxAmount: 0, discountAmount: 0, grandTotal: 0, oldDue: 0, totalDue: 0, amountPaid: 0, balanceDue: 0 }, settings: { notes: businessSettings?.defaultNotes || 'Thank you for choosing BillQyro! Payment is expected within due date.', terms: businessSettings?.terms || '', paymentStatus: 'Unpaid', orderStatus: 'Pending', paymentMethod: 'Cash', paymentNote: '' } } });
    }

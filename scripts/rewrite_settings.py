...
# The exact button we want to replace
new_button = """                      ].map((preset) => {
...
for file_path in files:
    ...
    # Replace the preset array
    # We find the start: `{[ ...`
    # and the end: `].map((preset) => {`
    content = re.sub(r'\{\[\s*\{\s*id:\s*\'classic\'[\s\S]*?\]\.map\(\(preset\)\s*=>\s*\{', new_presets, content)
    
    # Replace the button rendering block
    # from `const isSelected = themeColor === preset.id;`
    # until `);` before `})`
    content = re.sub(r'\]\.map\(\(preset\)\s*=>\s*\{[\s\S]*?const isSelected = themeColor === preset.id;[\s\S]*?return \([\s\S]*?<button[\s\S]*?key=\{preset\.id\}[\s\S]*?type="button"[\s\S]*?onClick=\{[\s\S]*?\}\}[\s\S]*?className=\{`w-full text-left[\s\S]*?`\}[\s\S]*?>[\s\S]*?<\/button>[\s\S]*?\);', new_button, content)
    
    # Fix 'light' and 'pink' to 'classic' for init state
    content = content.replace("useState('light')", "useState('classic')")
    content = content.replace("useState('pink')", "useState('classic')")
    
    # If the file has the old `settings.themeColor || ... || 'light'`, replace it with the robust check
    old_init = "setThemeColor(settings.themeColor || (settings.themePreset === 'dark' ? 'light' : settings.themePreset) || 'light');"
    old_init2 = "setThemeColor(settings.themeColor || (settings.themePreset === 'dark' ? 'classic' : settings.themePreset) || 'classic');"
    
    new_init = """      const validThemes = ['classic', 'rose', 'ocean', 'emerald', 'indigo', 'midnight', 'sakura', 'arctic', 'desert', 'lavender', 'obsidian', 'crimson'];
